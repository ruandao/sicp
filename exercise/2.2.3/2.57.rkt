...
(define (make-sum . a)
  (define (make-sums items)
    (cond ((sum? items) (make-sums (cdr items)))
          ((null? items) 0)
          ((not (pair? items)) items)
          (else (let ((a1 (make-sums (car items)))
                      (a2 (make-sums (cdr items))))
                  (cond ((and (number? a1) (number? a2)) (+ a1 a2))
                        ((eq? a1 0) a2)
                        ((eq? a2 0) a1)
                        (else (list '+ (getV a1) (getV a2))))))))
  (make-sums a))
...
(define (make-product . m)
  (define (make-products items)
    (cond ((null? items) 1)
          ((not (pair? items)) items)
          (else (let ((m1 (car items))
                      (m2 (make-products (cdr items))))
                  (cond ((or (eq? m1 0) (eq? m2 0)) 0)
                        ((eq? m1 1) m2)
                        ((eq? m2 1) m1)
                        ((and (number? m1) (number? m2)) (* m1 m2))
                        (else (list '* (getV m1) (getV m2))))))))
  (make-products m))
...
(define (make-exponentiation base exponent)
  (cond ((eq? exponent 1) (getV base))
        ((eq? exponent 0) 1)
        (else (list '** (getV base) (getV exponent)))))
